"""
sku_sync.py — Auto-sync daemon
Continuously monitors multiple Feishu SKU_match sheets (input sources).
When a new SKU is found with no Title, it looks it up and writes back Title/Author/Binding/Condition.
...
so a slow source (many pending SKUs) never blocks another source (few
pending SKUs) from picking up new rows.

SKU resolution is protected by a per-SKU lock + in-process cache: if two
source threads try to resolve the SAME SKU at nearly the same time, only
one of them actually calls the Google/OpenLibrary APIs and writes to the
DB cache — the other waits and reuses that result.

The in-process cache is intentionally NOT a rolling TTL per SKU. Instead,
...
while still avoiding duplicate API calls for the same SKU within a short
span of time.
...
Additionally, a low-frequency "stats" worker periodically reads the SKU
column from EVERY input source independently (not just pending rows),
counts how many times each SKU appears per source, and writes a summary
table (SKU | Title | Author | <source1> | <source2> | ... | Total) to a
...
from book_lookup import lookup_google, lookup_openlibrary
...
def clean_sku(raw) -> str:
    """
    Normalize a raw cell value into a plain digit-only SKU string.
    ...
    int(...) truncates it down to just "9". Two completely different SKUs
    mangled this way (e.g. "9.780593437476" and "9.781728297149") can both
    collapse to "9", which then makes resolve()'s per-SKU lock/cache treat
    ...
    if has_dot and not is_scientific:
        # A decimal point here almost certainly means the SKU got mangled
...
def read_input_sheet(spreadsheet_token: str, sheet_id: str) -> list[dict]:
    ...
    for i, row in enumerate(rows[1:], start=2):
        ...
        for j, col in enumerate(header):
            val = padded[j]
            rec[col] = clean_sku(val) if col == "sku" else ("" if val is None else str(val).strip())
        if rec.get("sku"):
            # Sanity check: a real SKU-10/SKU-13 is always 10 or 13 digits.
            # If clean_sku() produced something shorter/longer, the source
            # value was malformed (truncated float, partial OCR read, etc.)
            # — skip it rather than silently resolving/writing garbage that
            # could collide in the cache with an unrelated SKU.
            sku_len = len(rec["sku"])
            if sku_len not in (10, 13):
                log.warning(
                    f"  ⚠️  Row {i}: suspicious SKU '{rec['sku']}' "
                    f"(len={sku_len}, raw={padded[header.index('sku')]!r}) — skipping"
                )
                continue
            records.append(rec)
    return records
...
#     so that even outside of the per-SKU lock (belt-and-suspenders), two
#     writes can't interleave inside the read-then-append sequence.
_db_lock = threading.Lock()


def save_to_db(sku, title, author, binding, date, lang, pages, condition):
    ...
    try:
        with _db_lock:
            ...
            rows = payload.get("data", {}).get("valueRange", {}).get("values", [])
            existing_skus = [clean_sku(row[0]) for row in rows[1:] if row]
                ...
                def _do_append(force_refresh: bool):
                    r = requests.post(
                        ...
                        data=json.dumps({"valueRange": {
                            ...
                            "values": [[sku, title, author, binding, date, lang, pages, condition]],
                        }}),
                        timeout=10,
                    )
                    return r.json()
...
def lookup_db_cache(sku: str):
    ...
    try:
        ...
        rows = payload.get("data", {}).get("valueRange", {}).get("values", [])
        for row in rows[1:]:
            if row and clean_sku(row[0]) == sku:
...
# =============================================
# Resolve (merge both sources), thread-safe via per-SKU lock + memory cache
#
# If two source threads try to resolve the SAME sku at nearly the same
...
# makes resolution of any single SKU strictly one-at-a-time, in-process,
...
#   - within a day, repeated lookups of the same SKU never hit the API
...
#
# NOTE: this lock/cache is keyed by the SKU string produced by
# clean_sku(). It is only as correct as that key — if clean_sku() ever
# collapses two different SKUs to the same string (as it used to for
# malformed "9.780593437476"-style values, see clean_sku()'s docstring),
# this whole mechanism will confidently and silently treat them as one
# book. read_input_sheet() now filters out any SKU that isn't exactly
...
def _get_sku_lock(sku: str) -> threading.Lock:
    with _sku_locks_guard:
        lock = _sku_locks.get(sku)
        if lock is None:
            lock = threading.Lock()
            _sku_locks[sku] = lock
        return lock
...
def resolve(sku: str) -> tuple[str, str, str, str, str, str, str]:
    """
    Thread-safe wrapper: ensures only one thread ever actually performs
    the lookup + save_to_db for a given sku between scheduled cache
    resets. Other threads asking for the same sku will block briefly
    and then reuse the already-computed result.
    """
    with _memory_cache_lock:
        cached = _memory_cache.get(sku)
    if cached:
        log.info(f"  ⚡ {sku} → reused in-process cache (no duplicate API call)")
        return cached

    sku_lock = _get_sku_lock(sku)
    with sku_lock:
        # Another thread may have finished resolving this sku while we
        # were waiting for the lock — check again before doing any work.
        with _memory_cache_lock:
            cached = _memory_cache.get(sku)
        if cached:
            log.info(f"  ⚡ {sku} → reused result from concurrent lookup (waited on lock)")
            return cached

        result = _resolve_uncached(sku)

        with _memory_cache_lock:
            _memory_cache[sku] = result
        return result


def _resolve_uncached(sku: str) -> tuple[str, str, str, str, str, str, str]:
    """
    Actual resolve logic (DB cache -> Google/OpenLibrary -> merge -> save).
    Only ever called while holding that sku's lock, so it's safe even
    though it isn't itself thread-safe.
    """
    t, a, b, d, l, p, c = lookup_db_cache(sku)
    if t:
        return t, a, b, d, l, p, "DB"

    gb_title, gb_author, gb_binding, gb_date, gb_lang, gb_pages = lookup_google(sku)
    ol_title, ol_author, ol_binding, ol_date, ol_lang, ol_pages = lookup_openlibrary(sku)
    ...
    save_to_db(sku, title, author, binding, date, lang, pages, condition)
...
def _clear_memory_cache(reason: str):
    with _memory_cache_lock:
        ...
        _memory_cache.clear()
    log.info(f"[cache] Reset ({reason}) — cleared {count} cached SKU(s), will re-read from DB sheet as needed")
...
def sync_source(source: dict) -> int:
    ...
    log.info(f"[{name}] Found {len(pending)} SKU(s) to look up")
    count = 0
    for r in pending:
        sku    = r["sku"]
        row_num = r["_row"]
        try:
            title, author, binding, date, lang, pages, condition = resolve(sku)
            write_back(sp_token, sh_id, row_num, title, author, binding, condition)
            log.info(f"  ✅ [{name}] Row {row_num} | {sku} → {title} / {author} [{binding}] {date} ({condition})")
            count += 1
            time.sleep(0.5)
        except Exception as e:
            log.error(f"  ❌ [{name}] Row {row_num} | {sku} → Error: {e}")
...
# periodically re-reads the SKU column from EVERY input source (the
# full sheet, not just pending rows), counts how many times each SKU
# appears per source, and writes a summary table to a separate output
# sheet:
#
#   SKU | Title | Author | <source1 name> | <source2 name> | ... | Total
#
...
# made just for stats — if an SKU hasn't been resolved yet those
...
def _stats_sources() -> list[dict]:
    ...
    a testing/sandbox source that should still go through normal SKU
...
def compute_sku_frequency() -> dict:
    """
    Reads the SKU column of every stats-eligible input source independently
    and counts occurrences per source. Sources marked
    include_in_stats: false (e.g. a testing source) are skipped entirely.
    Returns: {sku: {source_name: count, ...}, ...}
    """
    freq_by_source = {}
    for source in _stats_sources():
        ...
        freq_by_source[source["name"]] = Counter(skus)

    all_skus = set()
    for counter in freq_by_source.values():
        all_skus.update(counter.keys())

    merged = {}
    for sku in all_skus:
        merged[sku] = {name: freq_by_source[name].get(sku, 0) for name in freq_by_source}
    return merged
...
def stats_worker():
    ...
    while True:
        try:
            ...
            write_stats(merged)
            log.info(f"[stats] Updated frequency for {len(merged)} distinct SKU(s) across {len(INPUT_SOURCES)} source(s)")
        except Exception as e:
...
def main():
    log.info("=" * 50)
    log.info("SKU Sync daemon started")
